...
class BuildTemplateKML:
    ...
    # -------------------------------------------------------------------------
    def generate(self):
        # Find the first waypoint's elevation_from_dsm
        base_index = 0
        action_group_id = 0
        touch_sky_count = 0

        # --- Add first Placemark with index 0: initial position with gimbal rotate ---
        if self.wpt_csv_properties and len(self.wpt_csv_properties) > 0:
            lat_y, lon_x, wpt_elevation_from_dsm, point_id = self.wpt_csv_properties[0]
            _, _, wpt_ellips_height_egm96 = transform_to_egm96(lat_y, lon_x, wpt_elevation_from_dsm)
            height_ellips = str(float(wpt_elevation_from_dsm) + float(config.touch_sky_altitude))
            height_egm96 = str(float(wpt_ellips_height_egm96) + float(config.touch_sky_altitude))
            self.addTouchSkyPlacemark(
                0, lat_y, lon_x, height_ellips, height_egm96,
                self.gimbal_pitch_rotate_angle, action_group_id, 0
            )
        ...
        for idx, (lat_y, lon_x, wpt_elevation_from_dsm, point_id) in enumerate(self.wpt_csv_properties):
            ...
            self.addTreePhotosPlacemark(
                base_index + 1, lat_y, lon_x, height_ellips, height_egm96, point_id, action_group_id, base_index + 1)
            ...
            if config.touch_sky and touch_sky_count >= config.touch_sky_interval:
                ...
                height_egm96 = str(float(wpt_ellips_height_egm96) + float(config.touch_sky_altitude))
                self.addTouchSkyPlacemark(
                    base_index + 2, lat_y, lon_x, height_ellips, height_egm96,
                    self.gimbal_pitch_rotate_angle, action_group_id, base_index + 2)

                base_index += 2  # photo + touchsky
                touch_sky_count = 0
            else:
                base_index += 1  # photo only
    ...
    def addTreePhotosPlacemark(self, idx, lat_y, lon_x, height_ellips, height_egm96, point_id, actionGroupId, actionGroupIndex):
        ...
        photo_actions = [DRONE_MODEL_CONFIG[config.drone_model]['photo_actions'][0]]
