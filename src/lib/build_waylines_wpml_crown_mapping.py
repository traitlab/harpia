...
class BuildWaylinesWPML:
    ...
    def generate(self):
        ...
        if self.wpt_csv_properties and len(self.wpt_csv_properties) > 0:
            ...
            height = str(float(wpt_elevation_from_dsm) + float(config.touch_sky_altitude))
            self.addTouchSkyPlacemark(0, lat_y, lon_x, height, point_id, self.gimbal_pitch_rotate_angle, action_group_id, 0)
        ...
        for idx, (lat_y, lon_x, wpt_elevation_from_dsm, point_id) in enumerate(self.wpt_csv_properties):
            ...
            self.addTreePhotosPlacemark(base_index + 1, lat_y, lon_x, height, point_id, action_group_id, base_index + 1)
            ...
            if config.touch_sky and touch_sky_count >= config.touch_sky_interval:
                action_group_id += 1
                height = str(float(wpt_elevation_from_dsm) + float(config.touch_sky_altitude))
                self.addTouchSkyPlacemark(base_index + 2, lat_y, lon_x, height, point_id, self.gimbal_pitch_rotate_angle, action_group_id, base_index + 2)
