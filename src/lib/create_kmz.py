...
class CreateKMZ:
    ...
    def __init__(self):
        # File path
        self.wpmz_dir = Path(config.output_folder) / config.base_name / f"wpmz"
        
        # Add datetime to filename when in debug mode
        if config.debug_mode == False:
            self.kmz_file_path = Path(config.output_folder) / config.base_name / f"{config.base_name}.kmz"
        else:
            # Get current datetime
            current_datetime = datetime.now().strftime("%Y%m%dT%H%M%S")            
            self.kmz_file_path = Path(config.output_folder) / config.base_name / f"{config.base_name}_{current_datetime}.kmz"
